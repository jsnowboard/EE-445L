...
void (*TimerAInitTask)(void);   // user function
...
// Outputs: none
void Timer0A_Init(void(*taska)(void), uint32_t perioda){long sr;
  sr = StartCritical(); 
  SYSCTL_RCGCTIMER_R |= 0x01;   // 0) activate TIMER0
  TimerAInitTask = taska;          // user function
//	TimerBInitTask = taskb;          // user function
  TIMER0_CTL_R = 0x00000000;    // 1) disable TIMER0A during setup
  TIMER0_CFG_R = TIMER_CFG_16_BIT;    // 2) configure for 16-bit mode
  TIMER0_TAMR_R = 0x00000002;   // 3) configure for periodic mode, default down-count settings
  TIMER0_TAILR_R = perioda-1;   // 4) reload value
...
void Timer0A_Handler(void){
  ...
  (*TimerAInitTask)();                // execute user task
}
